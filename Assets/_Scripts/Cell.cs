...
public class Cell
{
            ...
            if (availablePositions[buildingPosition])
            {
                ...
                else
                {
                    ...
                    if (!aux)
                    {
                        ...
                        //Bellow cells
                        Cell below =
                            BuildingSystem.Instance.GetCell(new Vector3(position.x, position.y - 3, position.z));
                        Cell belowLeft =
                        ...
                        //Same level cells
                        Cell left = BuildingSystem.Instance.GetCell(new Vector3(position.x - 3, position.y,
                            position.z));
                        Cell right =
                            BuildingSystem.Instance.GetCell(new Vector3(position.x + 3, position.y, position.z));
                        Cell front =
                            BuildingSystem.Instance.GetCell(new Vector3(position.x, position.y, position.z + 3));
                        Cell back = BuildingSystem.Instance.GetCell(new Vector3(position.x, position.y,
                            position.z - 3));
                        Cell frontRight =
    ...
    private void RegisterBuilding(BuildingPosition buildingPosition, GameObject building, BuildingType type, Vector3 rotation, bool preview = false)
    {
        if (!preview)
        {
    ...
    public bool RegisterBuilding(GameObject building, BuildingType type, Vector3 playerRotation, bool preview = false)
    {
        ...
        {
            RegisterBuilding(buildingPosition, building, type, playerRotation, preview);
